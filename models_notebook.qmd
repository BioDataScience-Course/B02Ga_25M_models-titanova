...
```{r}
microp_lm4 <- lm(data= microp, chla_fluo ~ plast_pe + temp_rec + plast_pe:temp_rec + 0)

tabularise(summary_(microp_lm4))  
```

```{r}
microp_lm5 <- lm(data= microp, chla_fluo ~ plast_pe * temp_rec)
tabularise(summary_(microp_lm5)) 
```

```{r}

AIC(microp_lm4, microp_lm5)
```

```{r}
chart$residuals(microp_lm5)
```
...
l'analyse des résidus montre que l'étendue des résidus est élevé (0.2) par rapport à celui des valeurs prédites (0.15), on observe aussi une non linéarité du modèle (A). Les résidus présente une non normalité mais la distribution est symétrique (en forme de S) (B). L'analyse des variances montre relativement une homoscédatiscité (C). Plusieurs valeurs extrêmes sont observées, la distance de Cook est importante pour certains points qui tirent la courbe de part et d'autre d'où l'effet de levier observé (D). D'après cette analyse des résidus, le modèle généralisé semble robuste pour modéliser nos données.
