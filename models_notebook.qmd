...
```{r étude des modèles}
microp_lm5.1 <- lm(data= microp, chla_fluo ~ plast_pe * temp_rec)
tabularise(summary_(microp_lm5.1))
microp_lm5.2 <- lm(data= microp, chla_fluo ~ plast_pe + temp_rec)
tabularise(summary_(microp_lm5.2))
```

```{r}
anova(microp_lm5.1, microp_lm5.2) |> tabularise()
```


```{r}
anova(microp_lm4.1, microp_lm5.1, test = "F") |> tabularise()
AIC(microp_lm4.1, microp_lm5.1)
```

```{r}
chart$residuals(microp_lm4.1)
```
...
$$
`r eq__(microp_lm4.1, use_coefs = TRUE)`
$$
