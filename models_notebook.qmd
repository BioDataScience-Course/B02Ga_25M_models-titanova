...
La masse en phytoplancton augmente en fonction de la concentration en polyéthylène théréphatalate. Une interprétation plausible de cette tendance est que la présence des microplastiques affecte le cycle de l'azote chez les populations microbiennes. On remarque que la température n'a que peu d'effet cependant, on retrouve des points à des températures plus hautes pour une concentration en microplastique à plus de 25g/cm³. Mais la température semble avoir un impacte secondaire sur le cycle de l'azote des populations microbiennes ici étudiées.
...
```{r}
lm(data=microp, fvfm ~ plast_tot + I(plast_tot)) -> microp_lm1_1
chart(microp_lm1_1)
lm(data=microp, fvfm ~ plast_tot + I(plast_tot^2)) -> microp_lm1_2
chart(microp_lm1_2)
lm(data=microp, fvfm ~ plast_tot + I(plast_tot^3)) -> microp_lm1_3
chart(microp_lm1_3)
```

```{r}
summary(microp_lm1_2) %>.%
  tabularise(.)
```
```
```
...
chart(microp_lm2_2)
microp_lm2_3 <- lm(data= microp,  chla_fluo ~plast_tot + I(plast_tot^3))
chart(microp_lm2_3)
```
...
anova(microp_lm4.1, microp_lm4.2) |> tabularise()
anova(microp_lm4.2, microp_lm4.3) |> tabularise()
AIC(microp_lm4.1, microp_lm4.2)
```
...
```{r}
microp_glm <- glm(data = microp, fvfm ~ ammonium * mesocosm, family = gaussian)
summary_(microp_glm) |> tabularise()
```

```{r}
microp_glmb <- glm(data = microp, fvfm ~ ammonium + mesocosm, family = gaussian)
summary_(microp_glmb) |> tabularise() 

```

```{r}
AIC(microp_glm,microp_glmb)
anova (microp_glm,microp_glmb, test = "F") |> tabularise()
```


```{r}
chart$residuals(microp_glm) 
...
$$
`r eq__(microp_glm, use_coefs = TRUE)`
$$

Toutes les variables sont significatives au seuil alpha de 5% . Le delta entre déviance totale et déviance résiduelle est faible. Le modèle n'est pas très bon. Les variables mésocosm et ammonium ont un impact sur la photosynthèse selon le modèle linéaire généralisé.

l'analyse des résidus montre que l'étendue des résidus est élevé (0.2) par rapport à celui des valeurs prédites (0.15), on observe aussi une non linéarité du modèle (A). Les résidus présente une non normalité mais la distribution est symétrique (en forme de S) (B). L'analyse des variances montre relativement une homoscédatiscité (C). Plusieurs valeurs extrêmes sont observées, la distance de Cookds est importante pour certains points qui tirent la courbe de part et d'autre d'où l'effet de levier observé (D).
...
```{r}
microp_glm1<- glm(data = microp, chla_fluo ~ plast_pet * temp, family = gaussian) 
summary_(microp_glm2) |> tabularise()

microp_glm2<- glm(data = microp, chla_fluo ~ plast_pet + temp, family = gaussian) 
summary_(microp_glm2) |> tabularise()

anova(microp_glm1, microp_glm2, test = "Chisq") |> tabularise()
...
Tous les paramètres de l'équation sont significativement différents de 0 au seuil alpha de 5% pour le modèle sans interaction. Le modèle avec interaction donne des paramètres non significatif au seuil alpha de 5%. Le modèle avec interaction (le plus complexe) a un AIC un peu plus faible (73.92 par rapport à 73.07). Pour déterminer quel modèle utiliser, une anova a été faite. Il n'y a significativement pas de différence au seuil alpha de 5% entre les deux modèles. Comme plusieurs variables sont non significatives avec le modèle le plus complexe, nous préférons choisir le modèle qui a un AIC un peu plus élevé (le plus simple). 
Le graphique représenté signale que les données sont relativement mal représentées par le modèle. L'intervalle de confiance du graphique étant élevé.
L'analyse des résidus montre que le modèle possède plusieurs problèmes. L'étalement des résidus (2.5) est élevé par rapport à l'étalement des valeurs prédites (2.5) (A). Il n'y a pas de normalité des résidus (B). Il n'y a pas d'homoscédasticité  des résidus (C). Il y a de plus présence de valeurs extrêmes avec des effets de levier et des distances de Cook élevées (D).
