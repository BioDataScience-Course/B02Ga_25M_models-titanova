...
```
```{r}
anova(microp_lm4, microp_lm5, test = "F") |> tabularise()
...
L'étendue des résidus est élevée (0.2) par rapport à l'étendue des valeurs prédites(0.15). La courbe du nuage de point est relativement non linéaire (A). Les résidus ne suivent pas une distribution normale (B) mais la distribution est symétrique (forme de S). La répartition des résidus montre relativement une homoscédasticité (C), une valeur extrême est observable. Il y a présence de quelques valeurs extrêmes, un effet de levier est observé, la distance Cooksd est importante (D). Après analyse des résidus, le modèle généralisé semble robuste pour modéliser nos données. 
