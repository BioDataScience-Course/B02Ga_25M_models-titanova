...
Le logiciel utilisé est R (version 4.4.1) (https://www.r-project.org) (`r R.version.string`)
Le jeu de données est légèrement remanié (traitement documenté dans le fichier `R/import_micropop.R`).
 
 et en particulier le package {modelit} version `r packageVersion("modelit")` dans la [SciViews Box 2025](https://www.sciviews.org/software/svbox/). [Saturn Cloud](https://saturncloud.io) (Linux Ubuntu 22.04) est utilisé pour exécuter le code et compiler la version finale de ce bloc-notes.
...
```{r chart2}
chart(data = microp, fvfm ~ plast_tot %col=% depth_rec)+
  geom_point()
```

La concentration de microplastique totale augmente l'efficacité de la photosynthèse des phytoplancton. La profondeur de l'océan ne semble pas avoir d'impact sur l'efficacité de la photosynthèse. Tout de même les phytoplanctons les moins efficaces sont ceux en grande profondeur.
...
```{r chart3}
chart(data=microp,plast_tot ~chla_fluo)+
  geom_point()
```
...
$$
Le modèle de l'efficacité de la concentration en microplastiques en fonction de l'efficacité de la photosynthèse du deuxième ordre a un R² assez faible et une p-value significative. Les valeurs p  de chaque paramètre sont aussi significatives. l'étude des résidus montre que l'étendue des résidus par rapport aux valeurs prédites est assez bonne. La distribution des résidus est normale, on observe quelques valeurs extrêmes. Il n'y a pas d'homoscédatiscité des résidus et l'effet de levier est conséquent ce qui est causé par plusieurs valeurs.
